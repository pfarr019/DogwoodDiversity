#note: need to change input parameters (ie alpha) directly on line 3

apparent = function (InputFile, MaxIdent=0.10, alpha=0.05, nloci=300, self=TRUE, plot=TRUE, Dyad=TRUE) {
  ...
  if (Dyad==TRUE){
    combined_output <- list(Triad_all = Out1, Triad_sig = Out2a, Triad_summary_pop = LogOut1,
                            Triad_summary_geno = LogOut2, Dyad_sig = Out3b, ThresholdT = ThresholdT, SortGD = SortGD)
  } else {
    combined_output <- list(Triad_all = Out1, Triad_sig = Out2a, Triad_summary_pop = LogOut1,
                            Triad_summary_geno = LogOut2, ThresholdT = ThresholdT, SortGD = SortGD)
  }
